...
import { saveChurchBrandKitAction, type ChurchBrandKitActionState } from "@/app/app/settings/actions";

type TypographyDirection = "match_site" | "graceled_defaults";

type ChurchBrandKitFormProps = {
...
const initialState: ChurchBrandKitActionState = {};
...
export function ChurchBrandKitForm({
  ...
  return (
    <form action={action} className="space-y-5 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
      <div>
        <h1 className="text-xl font-semibold">Church Brand Kit</h1>
        <p className="text-sm text-slate-600">Set your church-wide website, logo, palette, and typography defaults.</p>
      </div>
      ...
      <div className="flex items-center gap-4">
        ...
        >
          {pending ? "Saving church brand kit..." : "Save Church Brand Kit"}
        </button>
      </div>
    </form>
  );
