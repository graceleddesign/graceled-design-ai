...
export async function approveFinalDesignAction(projectId: string, generationId: string, optionKeyRaw: string): Promise<void> {
  ...
  if (!generationValidation.finalize.eligible) {
    throw new Error(buildProductionBlockedMessage("Finalize", generationValidation.finalize.invalidReasons));
  }

  const optionIndex = Math.max(0, normalizedOptionKey.charCodeAt(0) - 65);
  const designDoc = readCanonicalDesignDocFromOutput(generation.output);
  if (!designDoc) {
    throw new Error(buildProductionBlockedMessage("Finalize", ["missing_canonical_design_doc"]));
  }
